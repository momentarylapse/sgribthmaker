...
void SIAddPackageHui()
{
	...
	add_class(TypeHuiWindow);
		...
		class_add_func("__delete__",		TypeVoid,		mf(&HuiWindow::__delete__));
	...
	//add_func("GetInterior",									TypeIRect,		1,	TypePointer,"win");
		...
		class_add_func("EventX",						TypeVoid,		mf(&HuiWindow::EventX));
			...
			func_add_param("func",			TypePointer);
		class_add_func("OnMouseMove", TypeVoid, mf(&HuiWindow::OnMouseMove));
		class_add_func("OnMouseWheel", TypeVoid, mf(&HuiWindow::OnMouseWheel));
		class_add_func("OnLeftButtonDown", TypeVoid, mf(&HuiWindow::OnLeftButtonDown));
		class_add_func("OnMiddleButtonDown", TypeVoid, mf(&HuiWindow::OnMiddleButtonDown));
		class_add_func("OnRightButtonDown", TypeVoid, mf(&HuiWindow::OnRightButtonDown));
		class_add_func("OnLeftButtonUp", TypeVoid, mf(&HuiWindow::OnLeftButtonUp));
		class_add_func("OnMiddleButtonUp", TypeVoid, mf(&HuiWindow::OnMiddleButtonUp));
		class_add_func("OnRightButtonUp", TypeVoid, mf(&HuiWindow::OnRightButtonUp));
		class_add_func("OnDoubleClick", TypeVoid, mf(&HuiWindow::OnDoubleClick));
		class_add_func("CanClose", TypeBool, mf(&HuiWindow::CanClose));
