...
bool _type_match(Class *type, bool is_class, Class *wanted);
bool _direct_type_match(Class *a, Class *b);
bool direct_type_match_x(Class *given, Class *wanted);
bool type_match_with_cast(Class *type, bool is_modifiable, Class *wanted, int &penalty, int &cast);
...
Command *SyntaxTree::CheckParamLink(Command *link, Class *type, const string &f_name, int param_no)
{
	...
	if ((wt->is_pointer) and (wt->is_silent)){
		if (direct_type_match_x(pt, wt->parent)){

			return ref_command(link);
		}else if ((pt->is_pointer) and (direct_type_match_x(pt->parent, wt->parent))){
			// silent Ref & of *
	...
	// normal type cast
	}else if (!direct_type_match_x(pt, wt)){
		int pen, tc;
		/*int tc = -1;
		...
		if (type_match_with_cast(pt, false, wt, pen, tc))
...
Command *SyntaxTree::GetFunctionCall(const string &f_name, Array<Command> &links, Block *block)
{
	...
	for (Command &link: links){
		...
		for (int p=0; p<params.num; p++){
			if (!direct_type_match_x(wanted_types[p], params[p]->type)){
				printf("no match: %s / %s\n", wanted_types[p]->name.c_str(), params[p]->type->name.c_str());
				ok = false;
				break;
			}
		}
			...
			if ((links[0].kind == KIND_FUNCTION) or (links[0].kind == KIND_VIRTUAL_FUNCTION) or (links[0].kind == KIND_STATEMENT)){
				...
				for (int i=0;i<operators.num;i++)
					if (po == operators[i].primitive_id)
						if ((operators[i].param_type_1 == TypeVoid) and (direct_type_match_x(p2, operators[i].param_type_2))){
							o = i;
				...
				if (!ok){
					...
					for (int i=0;i<operators.num;i++)
						if (po == operators[i].primitive_id)
							if ((operators[i].param_type_1 == TypeVoid) and (type_match_with_cast(p2, false, operators[i].param_type_2, pen2, c2))){
								ok = true;
								if (pen2 < pen_min){
									r = operators[i].return_type;
									o = i;
...
bool type_match_with_cast(Class *given, bool is_modifiable, Class *wanted, int &penalty, int &cast)
{
	penalty = 0;
	cast = -1;
	if (direct_type_match_x(given, wanted))
	    return true;
	...
	for (int i=0;i<TypeCasts.num;i++)
		if ((direct_type_match_x(TypeCasts[i].source, given)) and (direct_type_match_x(TypeCasts[i].dest, wanted))){
	...
	for (ClassFunction &f: pp1->function)
		if (f.name == op_func_name){
			...
			if (f.param_types[0]->is_pointer and f.param_types[0]->is_silent){
				if (direct_type_match_x(p2, f.param_types[0]->parent)){
			...
			}else if (direct_type_match_x(p2, f.param_types[0])){
				Command *inst = ref_command(param1);
				if (p1 == pp1)
					op = add_command_classfunc(&f, inst);
				else
...
/*	for (int i=0;i<PreOperators.num;i++)
		if (op_no == PreOperators[i].primitive_id)
			if (direct_type_match_x(p1, PreOperators[i].param_type_1) and direct_type_match_x(p2, PreOperators[i].param_type_2)){
	...
	/*for (int i=0;i<PreOperators.num;i++)
		if (op_no == PreOperators[i].primitive_id)
			if (type_match_with_cast(p1, left_modifiable, PreOperators[i].param_type_1, pen1, c1) and type_match_with_cast(p2, false, PreOperators[i].param_type_2, pen2, c2))
	...
	foreachi(ClassFunction &f, p1->function, i)
		if (f.name == op_func_name)
			if (type_match_with_cast(p2, false, f.param_types[0], pen2, c2))
				if (pen2 < pen_min){
					op_found = i;
					pen_min = pen2;
					c1_best = -1;
...
void SyntaxTree::ParseSpecialCommandForall(Block *block)
{
	...
	Command *cmd_var_assign = add_command_operator(for_var_ref, array_el_ref, OPERATOR_POINTER_ASSIGN);
