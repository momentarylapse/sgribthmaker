...
void add_type_cast(int penalty, Class *source, Class *dest, const string &cmd, void *func)
{
	...
	if (c.func_no < 0){
#ifdef _X_USE_HUI_
		hui::HuiErrorBox(NULL, "", "add_type_cast (ScriptInit): " + string(cmd) + " not found");
		hui::HuiRaiseError("add_type_cast (ScriptInit): " + string(cmd) + " not found");
#else
