/*
 * HuiControlTreeViewGtk.cpp
 *
 *  Created on: 18.06.2013
 *      Author: michi
 */

#include "HuiControlTreeViewGtk.h"
#include "../hui.h"
...
void CreateTypeList();
void configure_tree_view_columns(HuiControlGtk *c, GtkWidget *view);
...
HuiControlTreeViewGtk::HuiControlTreeViewGtk(const string &title, const string &id, HuiWindow *win) :
	HuiControlGtk(HuiKindTreeView, id)
{
...
HuiControlTreeViewGtk::~HuiControlTreeViewGtk()
{
	// TODO Auto-generated destructor stub
}

string HuiControlTreeViewGtk::GetString()
{
}

void HuiControlTreeViewGtk::SetString(const string &str)
{
	AddString(str);
}
...
void HuiControlTreeViewGtk::AddString(const string& str)
{
	GtkTreeIter iter;
	GetPartStrings("", str);
...
void HuiControlTreeViewGtk::SetInt(int i)
{
...
int HuiControlTreeViewGtk::GetInt()
{
	GtkTreeSelection *sel = gtk_tree_view_get_selection(GTK_TREE_VIEW(widget));
...
void HuiControlTreeViewGtk::AddChildString(int parent_row, const string& str)
{
	GtkTreeIter iter;
	GetPartStrings("", str);
	GtkTreeStore *store = GTK_TREE_STORE(gtk_tree_view_get_model(GTK_TREE_VIEW(widget)));
...
void HuiControlTreeViewGtk::ChangeString(int row, const string& str)
{
	GetPartStrings("", str);
	GtkTreeStore *store = GTK_TREE_STORE(gtk_tree_view_get_model(GTK_TREE_VIEW(widget)));
	/*for (int j=0;j<PartString.num;j++)
...
string HuiControlTreeViewGtk::GetCell(int row, int column)
{
	GtkTreeModel *store = gtk_tree_view_get_model(GTK_TREE_VIEW(widget));
	return tree_get_cell(store, _item_[row], column);
}

void HuiControlTreeViewGtk::SetCell(int row, int column, const string& str)
{
	GtkTreeStore *store = GTK_TREE_STORE(gtk_tree_view_get_model(GTK_TREE_VIEW(widget)));
	if (gtk_tree_store_iter_is_valid(store, &_item_[row]))
...
Array<int> HuiControlTreeViewGtk::GetMultiSelection()
{
	Array<int> sel;
...
void HuiControlTreeViewGtk::SetMultiSelection(Array<int>& sel)
{
	GtkTreeSelection *s = gtk_tree_view_get_selection(GTK_TREE_VIEW(widget));
	gtk_tree_selection_set_mode(s, GTK_SELECTION_MULTIPLE);
...
void HuiControlTreeViewGtk::Reset()
{
	GtkTreeStore *store = GTK_TREE_STORE(gtk_tree_view_get_model(GTK_TREE_VIEW(widget)));
...
void HuiControlTreeViewGtk::Expand(int row, bool expand)
{
}

void HuiControlTreeViewGtk::ExpandAll(bool expand)
{
	if (expand)
...
bool HuiControlTreeViewGtk::IsExpanded(int row)
{
