/*
 * HuiControlListViewGtk.cpp
 *
 *  Created on: 17.06.2013
 *      Author: michi
 */

#include "HuiControlListViewGtk.h"
#include "../hui.h"
...
{
	HuiControlListViewGtk *c = (HuiControlListViewGtk*)data;
...
HuiControlListViewGtk::HuiControlListViewGtk(const string &title, const string &id, HuiWindow *win) :
	HuiControlGtk(HuiKindListView, id)
{
...
HuiControlListViewGtk::~HuiControlListViewGtk() {
	// TODO Auto-generated destructor stub
}

string HuiControlListViewGtk::GetString()
{
}

void HuiControlListViewGtk::SetString(const string &str)
{
	AddString(str);
}
...
void HuiControlListViewGtk::AddString(const string& str)
{
	GtkTreeIter iter;
	GetPartStrings("", str);
...
void HuiControlListViewGtk::SetInt(int i)
{
...
int HuiControlListViewGtk::GetInt()
{
	GtkTreeSelection *sel = gtk_tree_view_get_selection(GTK_TREE_VIEW(widget));
...
void HuiControlListViewGtk::ChangeString(int row, const string& str)
{
	GetPartStrings("", str);
	GtkListStore *store = GTK_LIST_STORE(gtk_tree_view_get_model(GTK_TREE_VIEW(widget)));
...
string HuiControlListViewGtk::GetCell(int row, int column)
{
	GtkTreeModel *store = gtk_tree_view_get_model(GTK_TREE_VIEW(widget));
	return tree_get_cell(store, _item_[row], column);
}

void HuiControlListViewGtk::SetCell(int row, int column, const string& str)
{
	GtkListStore *store = GTK_LIST_STORE(gtk_tree_view_get_model(GTK_TREE_VIEW(widget)));
	if (gtk_list_store_iter_is_valid(store, &_item_[row]))
...
Array<int> HuiControlListViewGtk::GetMultiSelection()
{
	Array<int> sel;
...
void HuiControlListViewGtk::SetMultiSelection(Array<int>& sel)
{
	GtkTreeSelection *s = gtk_tree_view_get_selection(GTK_TREE_VIEW(widget));
	gtk_tree_selection_set_mode(s, GTK_SELECTION_MULTIPLE);
...
void HuiControlListViewGtk::Reset()
{
	GtkListStore *store = GTK_LIST_STORE(gtk_tree_view_get_model(GTK_TREE_VIEW(widget)));
