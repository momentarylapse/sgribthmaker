...
#endif
#ifdef OS_LINUX
	//XQueryTextExtents(hui::hui_x_display, );
...
XVisualInfo *choose_visual()
{
	...
	int screen = DefaultScreen(hui::hui_x_display);

	hui::HuiControl *c = NixWindow->_get_control_(NixControlID);
	hui::HuiControlDrawingArea *da = dynamic_cast<hui::HuiControlDrawingArea*>(c);
	...
	XVisualInfo *vi = glXChooseVisual(hui::hui_x_display, screen, attrListDblAccum);
	if (vi){
		msg_db_m("-doublebuffered + accum", 1);
		NixGLDoubleBuffered = true;
	}else{
		vi = glXChooseVisual(hui::hui_x_display, screen, attrListDbl);
		if (vi){
			msg_db_m("-doublebuffered", 1);
			NixGLDoubleBuffered = true;
		}else{
			msg_error("only singlebuffered visual!");
			vi = glXChooseVisual(hui::hui_x_display, screen, attrListSgl);
	...
	if (NixGLDoubleBuffered)
		fb_conf = glXChooseFBConfig(hui::hui_x_display, screen, attrListDbl, &n_fb_conf);
	else
		fb_conf = glXChooseFBConfig(hui::hui_x_display, screen, attrListSgl, &n_fb_conf);
	for (int i=0;i<n_fb_conf;i++){
		int value;
		if (glXGetFBConfigAttrib(hui::hui_x_display, fb_conf[i], GLX_VISUAL_ID, &value) == Success)
...
void NixInit(const string &api, hui::HuiWindow *win, const string &id)
{
	NixUsable = false;
	if (!msg_inited)
		msg_init();
	...
	XSetErrorHandler(xerrorhandler);
		...
		int NumModes;
		screen = DefaultScreen(hui::hui_x_display);
		XF86VidModeGetAllModeLines(hui::hui_x_display, screen, &NumModes, &modes);
		original_mode = modes[0];
		NixDesktopWidth = modes[0]->hdisplay;
		NixDesktopHeight = modes[0]->vdisplay;
		NixDesktopDepth = modes[0]->hdisplay;
	#else
		NixDesktopWidth = XDisplayWidth(hui::hui_x_display, 0);
		NixDesktopHeight = XDisplayHeight(hui::hui_x_display, 0);
		NixDesktopDepth = 32;
	#endif
	/*if (NixWindow){
		Window SomeWindow;
		...
		XGetGeometry(hui::hui_x_display,GDK_WINDOW_XWINDOW(NixWindow->window->window),&SomeWindow,&x,&y,&w,&h,&borderDummy,&x_depth);
...
void NixKill()
{
	...
	if (NixFullscreen){
			...
			/*bool b=XF86VidModeSwitchToMode(hui::hui_x_display,screen,original_mode);
			XFlush(hui::hui_x_display);
			XF86VidModeSetViewPort(hui::hui_x_display,screen,0,0);*/
		#endif
	}
	if (NixWindow){
...
//	msg_write(p2s((HWND)GDK_WINDOW_HWND(gtk_widget_get_window(NixWindow->gl_widget))));
...
#if 0
	//Colormap cmap;
		_hui::hui_x_display_ = GDK_WINDOW_XDISPLAY(NixWindow->gl_widget->window);
		printf("display %p\n", hui::hui_x_display);
			#ifdef NIX_ALLOW_FULLSCREEN
				XF86VidModeModeInfo **modes;
				int num_modes;
	...
	//XSetWindowAttributes attr;
		msg_write("a2");
				XF86VidModeGetAllModeLines(hui::hui_x_display, screen, &num_modes, &modes);
				msg_write("b");
		...
		msg_write("c");
			...
			context = glXCreateContext(hui::hui_x_display, vi, 0, GL_TRUE);
	...
	//Window win=GDK_WINDOW_XWINDOW(NixWindow->window->window);
		...
		context = glXCreateContext(hui::hui_x_display, vi, 0, GL_TRUE);
		glXMakeCurrent(hui::hui_x_display, win, context);
    ...
    /* get a connection */
    _hui::hui_x_display_ = XOpenDisplay(0);
    screen = DefaultScreen(hui::hui_x_display);
//    XF86VidModeQueryVersion(hui::hui_x_display, &vidModeMajorVersion, &vidModeMinorVersion);
...
//    XF86VidModeGetAllModeLines(GLWin.dpy, GLWin.screen, &modeNum, &modes);
   ...
   // printf("glX-Version %d.%d\n", glxMajorVersion, glxMinorVersion);
    ...
    /* create a color map */
		...
		printf("screen %d\n", DefaultScreen(hui::hui_x_display));
    XSetWindowAttributes attr;
		Window root = RootWindow(hui::hui_x_display, 0);
    attr.colormap = XCreateColormap(hui::hui_x_display, root, vi->visual, AllocNone);
    attr.border_pixel = 0;

int event_mask = ExposureMask | KeyPressMask | ButtonPressMask |
            StructureNotifyMask;
        win = XCreateWindow(hui::hui_x_display, RootWindow(hui::hui_x_display, vi->screen),
            0, 0, 800, 600, 0, vi->depth, InputOutput, vi->visual,
            CWBorderPixel | CWColormap | CWEventMask, &attr);
        /* only set window title and handle wm_delete_events if in windowed mode */
        Atom wmDelete = XInternAtom(hui::hui_x_display, "WM_DELETE_WINDOW", True);
        XSetWMProtocols(hui::hui_x_display, win, &wmDelete, 1);
        XSetStandardProperties(hui::hui_x_display, win, "test",
            "test", None, NULL, 0, NULL);
        XMapRaised(hui::hui_x_display, win);


		
			if ((long)win < 1000)
				msg_error("no GLX window found");
			//XSelectInput(hui::hui_x_display,win, ExposureMask | KeyPress | KeyReleaseMask | StructureNotifyMask);

			// set video mode
			if (NixFullscreen){
				/*XF86VidModeSwitchToMode(hui::hui_x_display, screen, modes[best_mode]);
				XFlush(hui::hui_x_display);
				int x, y;
				XF86VidModeGetViewPort(hui::hui_x_display, screen, &x, &y);
				printf("view port:    %d %d\n", x, y);
				XF86VidModeSetViewPort(hui::hui_x_display, screen, 0, 0);
				XFlush(hui::hui_x_display);
				XF86VidModeGetViewPort(hui::hui_x_display, screen, &x, &y);
				printf("view port:    %d %d\n", x, y);
				//XF86VidModeSetViewPort(hui::hui_x_display, screen, 0, NixDesktopHeight - yres);
				printf("Resolution %d x %d\n", modes[best_mode]->hdisplay, modes[best_mode]->vdisplay);
				XFree(modes);*/
				int r = system(format("xrandr --size %dx%d", xres, yres));
				XWarpPointer(hui::hui_x_display, None, win, 0, 0, 0, 0, xres / 2, yres / 2);
			}
		msg_write("e");
			glXMakeCurrent(hui::hui_x_display, win, context);
#endif
		if (glXIsDirect(hui::hui_x_display, context)){
...
//	NixOGLFontDPList=glGenLists(256);
    ...
    font = fontInfo->fid;*/ 
		...
		x_font=XLoadQueryFont(hui::hui_x_display,"*century*medium-r-normal*--14*");
		if (!x_font)
			x_font = XLoadQueryFont(hui::hui_x_display,"*medium-r-normal*--14*");
		if (!x_font)
			x_font = XLoadQueryFont(hui::hui_x_display,"*--14*");
		if (x_font){
