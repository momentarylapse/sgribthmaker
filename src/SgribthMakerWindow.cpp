	...
	in_update(this, [this] { update_menu(); })
{
	...
	set_key_code("new", hui::KEY_N + mod, "hui:new");
	event("open", [this] { open(); });
	set_key_code("open", hui::KEY_O + mod, "hui:open");
	event("open-directory", [this] { open_directory(); });
	set_key_code("open-directory", hui::KEY_O + mod + hui::KEY_SHIFT, "hui:open");
	event("save", [this] { on_save(); });
	set_key_code("save", hui::KEY_S + mod, "hui:save");
	event("save_as", [this] { on_save_as(); });
	set_key_code("save_as", hui::KEY_S + hui::KEY_SHIFT + mod, "hui:save_as");
	event("close", [this] { on_close_document(); });
	set_key_code("close", hui::KEY_W + mod, "hui:close");
	event("exit", [this] { on_exit(); });
	set_key_code("exit", hui::KEY_Q + mod, "hui:quit");
	//event("show_data", "", hui::KEY_D + mod, &ShowData);
	event("execute_command", [this] { execute_command_dialog(); });
	set_key_code("execute_command", hui::KEY_E + mod, "");
	event("find", [this] { execute_command_dialog(); });
	set_key_code("find", hui::KEY_F + mod, "");
	event("cut", [this] { on_cut(); });
	set_key_code("cut", hui::KEY_X + mod, "hui:cut");
	...
	set_key_code("uncomment", hui::KEY_D + mod + hui::KEY_SHIFT, "");
	event("indent", [this] { on_indent(); });
	set_key_code("indent", hui::KEY_TAB + mod, "");
	event("unindent", [this] { on_unindent(); });
	set_key_code("unindent", hui::KEY_TAB + mod + hui::KEY_SHIFT, "");
	event("reload", [this] { on_reload(); });
	set_key_code("reload", hui::KEY_R + mod, "hui:reload");
	event("undo", [this] { on_undo(); });
	set_key_code("undo", hui::KEY_Z + mod, "hui:undo");
	event("redo", [this] { on_redo(); });
	set_key_code("redo", hui::KEY_Z + hui::KEY_SHIFT + mod, "hui:redo");
	...
	set_key_code("compile_and_run_verbose", hui::KEY_F6 + mod);
	...
	set_key_code("auto-complete", mod + hui::KEY_SPACE);
...
void SgribthMakerWindow::on_close_document() {
	allow_doc_termination(cur_doc()).then([this] {
...
void SgribthMakerWindow::open() {
	hui::file_dialog_open(this, _("Open file"), working_dir_from_doc(cur_doc()), {"showfilter="+_("All (*.*)"), "filter=*"}).then([this] (const Path &filename) {
...
void SgribthMakerWindow::reload(Document *doc) {
	allow_doc_termination(doc).then([this, doc] {
		if (doc->filename) {
